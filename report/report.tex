...
Out tool therefore estimates the success rate of dictionary based bulk guessing attacks by assuming that our sample of $50\times 10^6$ guesses are representative of a larger scale attack. This relies on the assumption that the partial challenges are always 3 indeces long, and are randomly generated on every attempt in an atomic manner (i.e: no relation to previous challenge indeces). We also assume that the attack \& victim dictionaries are representative of real world passwords, and that our attack is not restricted by rate limitation (i.e: it is either offline or bypasses rate limitation policies).
